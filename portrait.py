...
class FaceSegmentation:
    ...
    def get_face_parts(self,image):
        # face_detector = facer.face_detector('retinaface/mobilenet', device=device)
        img = facer.hwc2bchw(facer.read_hwc(image)).to(device=device)

        f_detector = facer.face_detector('retinaface/mobilenet', device=device)
        with torch.inference_mode():
            faces = f_detector(img)
        # print(faces)
        facer.show_bchw(facer.draw_bchw(img, faces))
        f_parser = facer.face_parser('farl/lapa/448', device=device) # optional "farl/celebm/448"
        with torch.inference_mode():
            faces = f_parser(img, faces)
        seg_logits = faces['seg']['logits']
        seg_probs = seg_logits.softmax(dim=1)  # nfaces x nclasses x h x w
        n_classes = seg_probs.size(1)
        vis_seg_probs = seg_probs.argmax(dim=1).float()/n_classes*255
        vis_img = vis_seg_probs.sum(0, keepdim=True)
        #background, face, right_brow, left_brow, right_eye, left_eye, nose, upper_lip, inside_mouth, lower_lip, hair
        return faces['points'].numpy(), vis_img
        #right eye, lefteye, nose, left_lip_corner, right_lip_corner
